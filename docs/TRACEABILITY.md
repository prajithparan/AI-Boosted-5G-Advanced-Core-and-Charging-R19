...
| MDF2 process: X1-provisioned task store (which warrant a received PDU belongs to), Valkey state, HI2 transport to the LEMF (TS 102 232-1 6.4, TCP), queue between the receive thread and slow per-PDU work, fault reporting to the ADMF | TS 33.127 5.3.4; TS 33.128 5.3, 5.5 | -- | **not built** (the `nfs/li-mdf` increment) |
