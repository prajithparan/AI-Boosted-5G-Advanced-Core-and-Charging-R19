...
## NWDAF containing MTLF -- Nnwdaf_MLModelProvision, training, ONNX inference in the AnLF (ADR-0369)

Source: `specs/5G_APIs-REL-19/TS29520_Nnwdaf_MLModelProvision.yaml`, commit `bca84b60a37773133bcae97e5c6c0d10a93b47b6`;
TS 29.520 V19.7.0 §4.5 (`specs/3gpp/TS_29.520_j70.txt`). Stage 2: TS 23.288 V19.7.0 §5.1, §5C.1, §6.2A, §6.2B.5/.7, §6.2E.2, §6.5.3; TS 29.510 `NwdafInfo.mlAnalyticsList`.

| Procedure | TS clause | Source | Test |
|---|---|---|---|
| Role per instance (`anlf` / `mtlf` / `both`); NRF profile `nwdafInfo.eventIds` (AnLF) / `mlAnalyticsList[].mlAnalyticsIds` (MTLF) | TS 23.288 5.1 NOTE 1; TS 29.510 MlAnalyticsInfo | `nfs/nwdaf/src/main.cpp` (`role`), `mtlf.cpp` `nrf_profile_info` | `test_nwdaf_mtlf.cpp` (the MTLF discovers the AnLF by `eventIds` for the ADRF storage subscription) |
| Nnwdaf_MLModelProvision_Subscribe create (`POST /nnwdaf-mlmodelprovision/v1/subscriptions` → 201 + Location; `failEventReports` `UNAVAILABLE_ML_MODEL`; none accepted → 500 `UNAVAILABLE_ML_MODEL_FOR_ALLEVENTS`; `immRep` → `mLEventNotifs`) | TS 29.520 4.5.2.2.2 | `mtlf.cpp` `install_routes` | TrainsStoresProvisionsAndTheAnlfPredicts (refused / mixed / accepted) |
| Nnwdaf_MLModelProvision_Subscribe update (`PUT .../{subscriptionId}` → 200 / 404) | TS 29.520 4.5.2.2.3 | same | not asserted by a test this increment -- disclosed |
| Nnwdaf_MLModelProvision_Unsubscribe (`DELETE` → 204 / 404) | TS 29.520 4.5.2.3.2 | same | same test |
| Nnwdaf_MLModelProvision_Notify (`POST {notifUri}` `array(NwdafMLModelProvNotif)`: `mLModelAdrf{adrfId, storTransId}`, `mLFileAddr`, `modelUniqueId`, `modelProviderId`, `modelUpdateInd`, `addModelInfo{ACCURACY, accMLModel, trainInpInfos}`; `3gpp-Sbi-Callback: Nnwdaf_MLModelProvision_myNotification`) | TS 29.520 4.5.2.4.2; TS 23.288 6.2A.2 | `mtlf.cpp` `deliver`, `event_notif` | same test (first and re-trained notifications) |
| Training through the sidecar (`TrainingExecutor` / `SubprocessExecutor`; MLflow run per training; accuracy = correct / total within a tolerance; labelled synthetic bootstrap below `min_samples`) | CLAUDE.md AI pipelines; TS 23.288 5C.1 | `training_executor.cpp`, `nfs/nwdaf/training/train_nf_load.py` | same test (`data_source` synthetic_bootstrap then adrf; distinct MLflow run ids) |
| Training data through the ADRF: storage subscription towards the AnLF (`Nadrf_DataManagement_StorageSubscriptionRequest`, `nrfDataSub`, `dataSetTag`), retrieval by data set | TS 23.288 6.2E.2 step 8, 6.2B.3; TS 29.575 4.2.2.3.2, 4.2.2.5.2 | `mtlf.cpp` `ensure_storage_subscription`, `training_dataset` | same test (injected observations reach the training set: ≥ 40 real windows) |
| Model storage through the ADRF (`Nadrf_MLModelManagement` POST, `allowConsumerList` = subscribers; PUT to widen it) | TS 23.288 6.2B.5; TS 29.575 4.3.2.2 | `mtlf.cpp` `train`, `reconcile_consumers` | same test (the subscriber retrieves the model: real ONNX bytes) |
| Re-training when the data set grew / on interval, one replica at a time; `modelUpdateInd` | TS 23.288 6.2A.1 ("further training"), 6.2A.2 | `mtlf.cpp` `run` | same test |
| AnLF: one holder subscribes at the MTLF (heartbeat, takeover, unsubscribe on shutdown); Notify handled by every replica; model retrieved from the ADRF (`Nadrf_MLModelManagement_RetrievalRequest`) into ONNX Runtime | TS 23.288 6.2A.0/6.2A.1, 6.2B.7 | `ml_consumer.cpp`, `model_runtime.cpp`, `/nwdaf-inbound/v1/ml-models` | same test (predictions require the model) |
| NF_LOAD predictions for a future period (`ana-req.endTs` / `extraReportReq.endTs`): per-instance predicted load with `confidence`, `AnalyticsData.start/expiry`; statistics otherwise | TS 23.288 6.5.3 Tables 6.5.3-1/-2, 6.1.3 validity period | `main.cpp` `compute`, `model_runtime.cpp` `nf_load_features` | same test (future → confidence on every instance; present → none); `test_nwdaf_analytics.cpp` NwdafNfLoadModel.* |
| Nnwdaf_MLModelTraining, `Ue_Positioning`, ModelSharing / VFL features, `inferDataForModel`, `useCaseCxt`; per-event timestamps in the training series; multi-step horizon | TS 29.520 4.6, 5.4.8; TS 23.288 6.2C | -- | **not built** / **disclosed** (ADR-0369) |

## Nnwdaf_MLModelMonitor -- AnLF-assisted ML model accuracy monitoring (ADR-0370)

Source: `specs/5G_APIs-REL-19/TS29520_Nnwdaf_MLModelMonitor.yaml`, commit `bca84b60a37773133bcae97e5c6c0d10a93b47b6`;
TS 29.520 V19.7.0 §4.7 (`specs/3gpp/TS_29.520_j70.txt`). Stage 2: TS 23.288 V19.7.0 §5C.1, §6.2E.3; TS 29.510 `NFService.ipEndPoints`.

| Procedure | TS clause | Source | Test |
|---|---|---|---|
| Nnwdaf_MLModelMonitor_Register / _Deregister (MTLF serves `POST /nnwdaf-mlmodelmonitor/v1/registrations` → 201 + Location; `DELETE .../{registrationId}` → 204; `modelId` + oneOf `consumerId`/`consumerSetId`) | TS 29.520 4.7.2.2.2, 4.7.2.3.2; TS 23.288 6.2E.3.2 | `nfs/nwdaf/src/mtlf.cpp` `install_routes` | `test_nwdaf_mtlf.cpp` AccuracyBelowThresholdIsReportedAndDrivesRetraining (the AnLF registers; the MTLF's subscription proves it) |
| AnLF registers the model it uses (holder replica; re-registers on model change; deregisters on shutdown) | TS 23.288 6.2E.3.2 steps 1-4 | `ml_consumer.cpp` `register_use`, `deregister` | same test |
| MTLF resolves the AnLF's monitor endpoint from its NRF profile (`nfServices[nnwdaf-mlmodelmonitor].ipEndPoints`) and subscribes (`POST /subscriptions`, `accuThreshold`, `immRep`) | TS 23.288 6.2E.3.3 steps 0-2; TS 29.510 NFService | `mtlf.cpp` `anlf_monitor_base`, `reconcile_registrations`; `main.cpp` `run_nrf_lifecycle` (nfServices) | same test (profile endpoint asserted; re-training proves the subscription) |
| Nnwdaf_MLModelMonitor_Subscribe / update / Unsubscribe served by the AnLF (`POST` → 201 + `immReport`; `PUT` → 200; `DELETE` → 204/404) | TS 29.520 4.7.2.4.2, 4.7.2.4.3, 4.7.2.5.2 | `main.cpp` monitor routes | same test (POST 201, DELETE 204) |
| Accuracy: every prediction logged, judged against the next observed load, correct within `tolerance`; accuracy = correct / total, `inferenceNum`, mean deviation over the window | TS 23.288 5C.1 (+ NOTE 3); 6.2E.3.3 steps 4-5 | `accuracy_monitor.cpp` | same test (`mlModelAcc < 80`, `inferenceNum ≥ 3`, `deviation > 10`) |
...
## Lawful Interception -- X1 provisioning, NE server + keepalive, X2/X3 delivery client (ADR-0372), HI2/HI3 PS-PDU codec (ADR-0373), LI_HI2 mediation (ADR-0374), LI_X2/X3 receiving server (ADR-0375), LI_HI2 Delivery Function (ADR-0376), MDF2 process (ADR-0377)
...
| HI2/HI3 delivery envelope: `PS-PDU`/`PSHeader`/`PSIRIPayload`/`PSCCPayload` carrying a TS 33.128 payload in `threeGPP33128DefinedIRI [19]` / `threeGPP33128DefinedCC [23]`, compiled from a hand-derived 5G closure of the version43 module | ETSI TS 102 232-1 (LI-PS-PDU version43); TS 33.128 5.5 | `specs/etsi/102232-1/LI-PS-PDU-3GPP-subset.asn` -> `li_generated` (`asn_DEF_PS_PDU`) | ADR-0373; exercised through the facade below |
| LI_HI2 PS-PDU build: PSHeader (li-psDomainId, LIID, CommunicationIdentifier, sequence number, timeStamp + microSecondTimeStamp, timeStampQualifier) and one PSIRIPayload with IRI-Type | TS 33.128 5.5.1 table 5.5.1-1, 5.5.2 table 5.5.2-1; TS 102 232-1 5.2.x | `libs/li-core/src/hi2.cpp` `encode_iri_message`, `decode_iri_message` | `test_li_hi2.cpp` EncodesAndDecodesEveryHeaderField, CarriesTheTs102232DomainIdAndTheTs33128PayloadOid, RejectsAnOversizedLiid |
| xIRI -> IRI mediation: iRIPayloadOID `{4 19 19 7 3}`, the event alternative the POI chose transferred verbatim, mediatedFromIndicator on a release/version difference | TS 33.128 5.5.2 table 5.5.2-2 | `hi2.cpp` `mediate_xiri` | KeepsTheEventChoiceAndSwapsThePayloadOid, RefusesAnXiriOnlyEventAlternative |
| Table 5.5.1-1 mappings from the received LI_X2 PDU: NFID -> networkFunctionIdentifier, IPID -> extendedInterceptionPointID, PDU timestamp -> PSHeader timestamp with qualifier timeOfInterception(1) | TS 33.128 5.5.1, 5.5.2; TS 103 221-2 5.3.7/5.3.8/5.3.9 | `hi2.cpp` `mediate_x2_pdu` | TagsMatchedAndOtherTargetIdentifiersWithTheirProvenance, RefusesANonTgpp33128PayloadFormat |
| IRI Target Identifiers with provenance: matchedOn from the X2 Matched attribute, other from the X2 Other attribute | TS 33.128 5.5.5; TS 103 221-2 5.3.18/5.3.19 | `hi2.cpp` `fill_target_identifier`, `libs/li-core/src/x1.cpp` `parse_target_identifier_fragment` | TagsMatchedAndOtherTargetIdentifiersWithTheirProvenance, ParsesTheX2AttributeFragment, RefusesAnIdentifierWithNoAsn1Alternative |
| Provenance `lEAProvided` (from the X1 provisioning message) and `observed` (identifiers inside the xIRI event); payload aggregation (several IRI payloads per PS-PDU); LI_HI3 / MDF3 | TS 33.128 5.5.5, 5.5.3; TS 102 232-1 6.2.3 | -- | **not built** / **disclosed** (ADR-0374; `MediationContext.target_identifiers` is the seam for lEAProvided) |
| LI_X2/LI_X3 receiving server: mTLS with a required client certificate, stream reframing by Header/Payload Length, keepalive acknowledgement echoing the Sequence Number, table 5.4.1-1 validation before delivery | TS 103 221-2 5.2.3, 5.1, 6.2.4, table 5.4.1-1; TS 33.128 5.3 | `libs/li-core/src/x2x3_server.cpp` | `test_li_x2x3_server.cpp` ReceivesStreamedPdusOverMtls, ReassemblesAFrameSplitAcrossTwoWrites, AnswersKeepalivesAndDoesNotDeliverThem, RefusesAPeerWithoutAClientCertificate |
| End-to-end: xIRI sent over LI_X2 by the real client, received by the real server, mediated into the LI_HI2 PS-PDU a LEMF would receive | TS 33.128 5.3, 5.5 | `x2x3_client.cpp` -> `x2x3_server.cpp` -> `hi2.cpp` | MediatesAReceivedXiriIntoAnHi2PsPdu |
| LI_HI2 Delivery Function: open on start and reopen on any termination, retry a failed open at a configurable interval, report failures | ETSI TS 102 232-1 6.3.2 | `libs/li-core/src/hi2_client.cpp` | `test_li_hi2_client.cpp` DeliversPsPdusToTheLemf, ReopensTheConnectionAndResynchronisesAfterADrop |
| Cyclic buffer: PDUs retained after sending, re-sent on reconnect before new data, oldest overwritten and reported when full | TS 102 232-1 6.3.3 | `hi2_client.cpp` `send`, `run` | ReopensTheConnectionAndResynchronisesAfterADrop, OverwritesTheOldestPduWhenTheBufferIsFull |
| Session-layer keep-alives: keep-alive TRI at TIME1 with an incrementing sequence number, connection torn down if no keep-aliveResponse within TIME3 | TS 102 232-1 6.3.4 | `hi2_client.cpp` `send_keepalive`, `read_available` | SendsSessionLayerKeepalives |
| TRI session-layer PDUs in the PS-PDU envelope (keep-alive pair, PDU-acknowledgement pair) and payload routing | TS 102 232-1 6.3.4, 6.3.6 | `hi2.cpp` `encode_tri_message`, `decode_tri_message`, `payload_kind`; `specs/etsi/102232-1/LI-PS-PDU-3GPP-subset.asn` | `test_li_hi2.cpp` EncodesAndDecodesTheKeepalivePair, PayloadKindDistinguishesIriFromTri |
| Clause 6.4.3 options 1 and 2 (TCP-acknowledged / send-buffer accounting); option negotiation; PDU acknowledgement driving; buffer persistence across restart | TS 102 232-1 6.3.5, 6.3.6, 6.4.3 | -- | **not built** / **disclosed** (ADR-0376; option 3 implemented) |
| MDF2 process (`li-mdf`): X1-provisioned task/destination store in Valkey (which warrant a received PDU belongs to; ADR-0359 no in-process state), XID→warrant lookup, per-LIID sequence numbering, X2 xIRI mediated to LI_HI2 through one Delivery Function per Destination; the LIID is read from `listOfMediationDetails` (Annex C.2.2), not `productID` | TS 33.127 5.3.4; TS 103 221-1 5.1.2, Annex C.2.2; TS 33.128 5.3, 5.5; TS 102 232-1 5.2.5 | `nfs/li-mdf/src/main.cpp`, `nfs/li-mdf/src/task_store.cpp`, `libs/li-core/src/x1.cpp` (`MediationDetails`, `read_task_details`) | `test_li_mdf.cpp` ProvisionsOverX1AndDeliversAMediatedIriToTheLemf |
...
| Procedure | TS clause | Source | Test |
|---|---|---|---|
| AMFRegistration xIRI: registrationType/Result + SUPI + 5G-GUTI | TS 33.128 6.2.2.2.2 | `libs/li-core/src/xiri.cpp` `fill/extract(AMFRegistration)` | `test_li_core.cpp` Xiri.EncodesBerXiriPayloadWithModuleOid, X2PduCarriesXiriPayloadEndToEnd |
| AMFDeregistration xIRI: deregistrationDirection + accessType (identifiers C, correlated by XID) | TS 33.128 6.2.2.2.3 | `xiri.cpp` `fill/extract(AMFDeregistration)` | `test_li_core.cpp` Xiri.AmfDeregistrationRoundTripsBothDirectionsAndAccessTypes |
| AMFStartOfInterceptionWithRegisteredUE xIRI: registrationResult + SUPI + 5G-GUTI (M members) | TS 33.128 6.2.2.2.5 | `xiri.cpp` `fill/extract(AMFStartOfInterceptionWithRegisteredUE)` | `test_li_core.cpp` Xiri.AmfStartOfInterceptionWithRegisteredUeRoundTrips |
| AMFLocationUpdate xIRI + the shared Location codec: SUPI + Location.locationInfo.userLocation (NR{TAI,NCGI}/E-UTRA{TAI,ECGI} -> PLMNID + TAC + 36/28-bit cell id via a BIT STRING helper) | TS 33.128 6.2.2.2.4; TS 33.128 Location (LocationInfo/UserLocation/NR/EUTRALocation) | `xiri.cpp` `fill/extract(AMFLocationUpdate)`, `fill/extract_location`, `fill_plmnid/tai/ncgi/ecgi`, `set/get_bit_string` | `test_li_core.cpp` Xiri.AmfLocationUpdateRoundTripsNrAndEutraUserLocation |
| AMFIdentifierAssociation xIRI: SUPI + 5G-GUTI + Location (all M) | TS 33.128 6.2.2.2.7 (table 6.2.2.2.7-1) | `xiri.cpp` `fill/extract(AMFIdentifierAssociation)` | `test_li_core.cpp` Xiri.AmfIdentifierAssociationRoundTrips |
| AMFIdentifierDeassociation xIRI: SUPI + 5G-GUTI (Location OPTIONAL) | TS 33.128 6.2.2.2.7 (table 6.2.2.2.7-2) | `xiri.cpp` `fill/extract(AMFIdentifierDeassociation)` | `test_li_core.cpp` Xiri.AmfIdentifierDeassociationRoundTripsWithAndWithoutLocation |
| AMF IRI-POI module: LI_X1 provisioning listener (li_core::x1 NE server, off the SBI plane -- no NRF), process-lifetime target-identifier store, LI_X2 emit to the MDF2; config-driven, disabled by default | TS 33.127 6.2.2; TS 103 221-1 (LI_X1); TS 33.128 5.3 (LI_X2) | `nfs/amf/src/li_poi.cpp` | `test_li_amf_poi.cpp` ProvisionsOverX1AndEmitsRegistrationXiriForATarget |
...
| X1 per-task gating: TaskDetailsExtensions/IdentifierAssociationExtensions (IdentifierAssociationEventsGenerated = IdentifierAssociation / All), global-element and X1Extensions/IdentifierAssociation forms; 3GPP X1 extension XSD imported into the validation set | TS 33.128 table 6.2.2.1.1-1/-2; `urn_3GPP_ns_li_3GPPX1Extensions.xsd` (r19:v4); TS 103 221-1 TaskDetails | `libs/li-core/src/x1.cpp` `read_task_gating`; `specs/etsi/103221-1/x1-validation.xsd` | `test_li_x1_gating.cpp` LiX1Gating.* (11) |
