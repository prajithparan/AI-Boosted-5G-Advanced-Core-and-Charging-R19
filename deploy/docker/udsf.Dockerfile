# Multi-stage build for nfs/udsf (ADR-0400). Mirrors deploy/docker/eir.Dockerfile -- see
...
WORKDIR /build
COPY --from=builder /build/build/nfs/udsf/udsf /build/udsf

EXPOSE 7830/tcp 9530/tcp

ENTRYPOINT ["/build/udsf"]
