# Multi-stage build for nfs/mfaf (ADR-0365). Mirrors deploy/docker/eir.Dockerfile -- see
...
WORKDIR /build
COPY --from=builder /build/build/nfs/mfaf/mfaf /build/mfaf

EXPOSE 7800/tcp 9487/tcp

ENTRYPOINT ["/build/mfaf"]
