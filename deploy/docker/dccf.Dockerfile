# Multi-stage build for nfs/dccf (ADR-0366). Mirrors deploy/docker/eir.Dockerfile -- see
...
WORKDIR /build
COPY --from=builder /build/build/nfs/dccf/dccf /build/dccf

EXPOSE 7803/tcp 9489/tcp

ENTRYPOINT ["/build/dccf"]
