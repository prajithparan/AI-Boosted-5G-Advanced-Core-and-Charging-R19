# Multi-stage build for nfs/adrf (ADR-0367). Mirrors deploy/docker/eir.Dockerfile -- see
...
WORKDIR /build
COPY --from=builder /build/build/nfs/adrf/adrf /build/adrf

EXPOSE 7804/tcp 9490/tcp

ENTRYPOINT ["/build/adrf"]
