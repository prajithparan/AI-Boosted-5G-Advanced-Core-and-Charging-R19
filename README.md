...
| **Energy-efficiency analytics, federated learning (VFL)** | **VFL hook built; energy-efficiency blocked on missing R19 SBI** | VFL: the NWDAF `Nnwdaf_VFLTraining` / `Nnwdaf_VFLInference` subscription surface is built (ADR-0380); the federated-training coordination behind it is Phase D. Energy-efficiency: the frozen R19 Nnwdaf SBI has **no** energy `NwdafEvent`/DTO (TS 23.288 6.16 energy analytics is OAM / TS 28.552-sourced, off the service-based interface) -- it cannot be built as an Nnwdaf analytic without inventing an API, so it is flagged, not fabricated |
...
| 33.126 / 33.127 / 33.128 | Lawful Interception | **Core** | **Partial — still a deployment blocker.** Built and tested: the MDF2 mediation & delivery path (`nfs/li-mdf`: LI_X1 provisioning, LI_X2 xIRI intake, LI_HI2 delivery to the LEMF; ADR-0373–0377) and the **AMF IRI-POI**'s Registration event (`nfs/amf` `li_poi`, ADR-0378: LI_X1 provisioning → AMFRegistration xIRI over LI_X2 to the MDF2; all six approved AMF event codecs done, one event wired end-to-end). Still missing: the AMF's five other events (blocked on a user-location parse / X1 gating / a deregistration procedure), the POIs 33.127 requires in SMF, UPF, UDM, SMSF, NEF, NWDAF and **§7.22 the CHF**, the NRF SIRF, the ADMF, and MDF3/HI3 (content of communication). A licensed operator still cannot deploy without these. |
