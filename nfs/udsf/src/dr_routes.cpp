...
std::string from_ms(std::int64_t ms) {
    return sbi_core::format_rfc3339(
...
tl::expected<ParsedRecord, Response> parse_record_body(const Request& req) {
    ...
    if (!mp::is_multipart(ct)) {
        return tl::unexpected(problem(415,
                                      "Unsupported Media Type",
                                      "a Record is sent as multipart/mixed (TS 29.598 6.1.2.4.2)"));
    }
    auto parts = mp::parse_any(ct, req.body);
    if (!parts) {
        return tl::unexpected(problem(
            400, "Malformed multipart body", parts.error(), std::string("INVALID_MSG_FORMAT")));
    }
    ParsedRecord out;
    try {
        ...
        return tl::unexpected(problem(
            400, "Malformed JSON in meta part", e.what(), std::string("INVALID_MSG_FORMAT")));
    }
    ...
    for (std::size_t i = 1; i < parts->size(); ++i) {
        ...
        if (std::any_of(out.blocks.begin(), out.blocks.end(), [&](const Block& o) {
                return o.id == b.id;
            })) {
            return tl::unexpected(problem(400,
                                          "Bad Request",
                                          "duplicate blockId " + b.id,
...
std::pair<std::int64_t, std::int64_t> sub_due(const Doc& d) {
    ...
    const std::int64_t notify = sub.expiryNotification && *sub.expiryNotification > 0
                                    ? exp - *sub.expiryNotification * 1000
                                    : 0;
    return {exp, notify};
}
...
void add_dr_routes(sbi_core::http2::Server& server, Ctx& ctx) {
    ...
    server.add_route("GET", rec, instrument("GetRecord", [&ctx, pre](const Request& req) {
                         ...
                         if (c.if_match && !if_match_passes(*c.if_match, r->etag)) {
                             auto resp = extended_problem(412,
                                                          "Precondition Failed",
    ...
    server.add_route(
        "PUT", rec, instrument("CreateOrModifyRecord", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            auto tx = ctx.store.modify_record(
                *s, id, [&](const std::optional<Record>& before, Record& next) {
                    ...
                    if (!c.write_passes(before ? std::optional<std::string>(before->etag)
                                               : std::nullopt)) {
                        Response r = get_previous && before
                                         ? record_response(ctx, 412, *before, false)
                                         : empty(412);
                        if (before) {
    ...
    // ----------------------------------------------
    server.add_route(
        "PATCH", rec, instrument("PatchRecord", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            for (auto& it : *items) {
                ...
                if (!meta_path && !block_path) {
                    bad.report.push_back({it.path,
                                          std::string("paths start with /meta or are "
            ...
            auto tx = ctx.store.modify_record(
                *s, id, [&](const std::optional<Record>& before, Record& next) {
                    ...
                    if (!patched->is_object() || patched->size() != 2 ||
                        !patched->contains("meta") || !(*patched)["blocks"].is_object()) {
                    ...
                    next.meta = meta;
                    next.blocks.clear();
                    for (auto b = (*patched)["blocks"].begin(); b != (*patched)["blocks"].end();
                         ++b) {
                        ...
                        if (h.rfind(kOld, 0) == 0) {
                            ...
                            nb.content_type = p->content_type.empty() ? "application/octet-stream"
                                                                      : p->content_type;
    ...
    // ------------------------------------------------------------------
    server.add_route(
        "PATCH", rec + "/meta", instrument("UpdateMeta", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            auto tx = ctx.store.modify_record(
                *s, id, [&](const std::optional<Record>& before, Record& next) {
                    ...
                    auto res = apply_itemwise(before->meta, *items, [&](const json& m) {
                        ...
                        if (why.empty() && ctx.settings.max_record_ttl_seconds > 0 &&
                            m.contains("ttl") &&
                            to_ms(m["ttl"].get<std::string>()) >
                                now_ms() + ctx.settings.max_record_ttl_seconds * 1000) {
                            why = "TTL_VALUE_NOT_ALLOWED: the ttl exceeds the maximum this UDSF "
                                  "allows";
                        }
                        return why;
                    });
    ...
    server.add_route(
        "GET", rec + "/blocks", instrument("GetBlockList", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
    ...
    server.add_route(
        "PUT", blk, instrument("CreateOrModifyBlock", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            auto tx = ctx.store.modify_record(
                *s, id, [&](const std::optional<Record>& before, Record& next) {
                    ...
                    if (!c.write_passes(old ? std::optional<std::string>(old->etag)
                                            : std::nullopt)) {
                    ...
                    next.blocks.erase(std::remove_if(next.blocks.begin(),
                                                     next.blocks.end(),
    ...
    server.add_route(
        "DELETE", blk, instrument("DeleteBlock", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
    ...
    // ----------------------------------------------------------------
    server.add_route(
        "GET", recs, instrument("SearchRecord", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            if (!filter_text) {
                return problem(400,
                               "Bad Request",
                               "the filter query parameter is required",
                               std::string("MANDATORY_QUERY_PARAM_MISSING"));
            }
            auto expr = parse_filter_param(*filter_text);
            if (!expr) {
                return problem(
                    400, "Bad Request", expr.error(), std::string("INVALID_QUERY_PARAM"));
            }
            ...
            if (retrieve && *retrieve != "ONLY_META" && *retrieve != "META_AND_BLOCKS") {
                return problem(400,
                               "Bad Request",
                               "unknown RetrieveRecords value " + *retrieve,
                               std::string("INVALID_QUERY_PARAM"));
            }
            ...
            if (!limit || !max_kb) {
                return problem(400,
                               "Bad Request",
                               !limit ? limit.error() : max_kb.error(),
                               std::string("INVALID_QUERY_PARAM"));
            }
            ...
            if (!matched) {
                return problem(
                    400, "Bad Request", matched.error(), std::string("INVALID_QUERY_PARAM"));
            }
            if (matched->empty()) {
                return empty(204);
            }
            ...
            // CombinedSearchRetrieve (6.1.2.4.6): descriptor, then "<id>/meta" and
            // "<id>/<blockId>".
            std::vector<mp::Part> parts;
            std::size_t size = 0;
            ...
            parts.insert(parts.begin(),
                         mp::Part{"application/json",
                                  std::string("recordSearchResultDescriptor"),
                                  json(d).dump()});
    ...
    server.add_route(
        "DELETE", recs, instrument("BulkDeleteRecords", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            for (const auto& id : *matched) {
                try {
                    auto tx = ctx.store.modify_record(
                        *s, id, [](const std::optional<Record>& b, Record&) {
            ...
            if (!failed.empty() && !partial) {
                return problem(
                    ...
                    std::to_string(failed.size()) +
                        " matching record(s) could not be "
            ...
            if (deleted.empty()) {
                return problem(500,
                               "Internal Server Error",
                               "no matching record could be deleted",
                               std::string("SYSTEM_FAILURE"));
            }
    ...
    server.add_route(
        "GET", subs, instrument("GetNotificationSubscriptions", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            if (!limit) {
                return problem(
                    400, "Bad Request", limit.error(), std::string("INVALID_QUERY_PARAM"));
            }
    ...
    server.add_route(
        "GET", sub, instrument("GetNotificationSubscription", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            Response r = json_response(
                200, sub_body(*d, negotiate(kDrFeatures, query(req, "supported-features"))));
    ...
    server.add_route(
        "PUT",
        sub,
        instrument("CreateAndUpdateNotificationSubscription", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
    ...
    server.add_route(
        "PATCH", sub, instrument("UpdateNotificationSubscription", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
    ...
    server.add_route(
        "DELETE",
        sub,
        instrument("DeleteNotificationSubscription", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            // (the name "client-id" itself is not on the wire). A JSON-encoded client-id=... is
            // also accepted (lenient; ADR-0401).
            ...
            if (auto j = query(req, "client-id")) {
                try {
                    ...
                    return problem(400,
                                   "Bad Request",
                                   std::string("invalid client-id: ") + e.what(),
                                   std::string("INVALID_QUERY_PARAM"));
                }
            } else {
            ...
            if (!cid.nfId && !cid.nfSetId) {
                return problem(400,
                               "Bad Request",
                               "client-id (nfId and/or nfSetId) is required",
                               std::string("MANDATORY_QUERY_PARAM_MISSING"));
            }
            const bool get_previous = query_flag(req, "get-previous");
            const auto c = conditions(req);
                ...
                *s,
                ...
                [&](const std::optional<Doc>& before, Doc&) {
                    ...
                    if (client_key(before->body.at("clientId").get<sbi_gen::ClientId>()) !=
                        ...
                        early = problem(403,
                                        "Forbidden",
                                        "client-id does not own this subscription",
                                        std::string("SUBSCRIPTION_EXISTS"));
                        return Verdict::Abort;
                    }
                    if (!c.write_passes(before->etag)) {
    ...
    server.add_route("GET", schema, instrument("GetMetaSchema", [&ctx, pre](const Request& req) {
                         ...
                         // TS table 6.1.3.9.3.1-3: MetaSchema as JSON. The YAML's 200 references
                         // the multipart Record response instead -- a YAML defect, see ADR-0401.
    ...
    server.add_route(
        "PUT", schema, instrument("CreateOrModifyMetaSchema", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            try {
                ...
                return problem(400,
                               "Bad Request",
                               std::string("invalid MetaSchema: ") + e.what(),
                               std::string("INVALID_MSG_FORMAT"));
            }
            if (ms.schemaId != id) {
                return problem(400,
                               "Bad Request",
                               "schemaId in the body differs from the URI",
                               std::string("INVALID_MSG_FORMAT"));
            }
            for (const auto& t : ms.metaTags) {
                ...
                if ((k == "SEARCH_KEY" || k == "SEARCH_AND_COUNT_KEY") && !t.sort) {
                    return problem(400,
                                   "Bad Request",
                                   "TagType.sort shall be present for " + k +
                                       " (table 6.1.6.2.16-1)",
                                   std::string("MANDATORY_IE_MISSING"));
                }
            }
            ...
            std::optional<Response> early;
            auto tx =
                ctx.store.modify_schema(*s, id, [&](const std::optional<Doc>& before, Doc& next) {
                    early.reset();
                    if (!c.write_passes(before ? std::optional<std::string>(before->etag)
                                               : std::nullopt)) {
                        Response r =
                            get_previous && before ? json_response(412, before->body) : empty(412);
                        if (before) {
                            r.headers.emplace("etag", before->etag);
                        }
                        early = r;
    ...
    server.add_route(
        "DELETE", schema, instrument("DeleteMetaSchema", [&ctx, pre](const Request& req) {
            auto s = pre(req);
            if (!s) {
                return s.error();
            }
            ...
            if (ctx.store.timers_using_schema(*s, id) > 0) {
                return problem(403,
                               "Forbidden",
                               "the meta schema is still referenced by timers",
