...
json blocks_json(const Record& r) {
    ...
    for (const auto& b : r.blocks) {
        out.push_back(
            {{"id", b.id}, {"ct", b.content_type}, {"cte", b.cte}, {"b64", base64_encode(b.data)}});
    }
    return out;
}
...
void append_blocks(std::vector<mp::Part>& parts, const json& blocks) {
    for (const auto& b : blocks) {
        mp::Part p{b.at("ct").get<std::string>(),
                   b.at("id").get<std::string>(),
...
std::int64_t to_ms(const std::string& rfc3339) {
    ...
    return tp ? std::chrono::duration_cast<std::chrono::milliseconds>(tp->time_since_epoch())
                    .count()
              : 0;
}
    ...
    for (const auto& id : store.claim_due(s, "tmr", now, kBatch)) {
        ...
        auto tx = store.modify_timer(
            ...
            [&](const std::optional<Doc>& before, Doc& next) {
                ...
                fired["expires"] = sbi_core::format_rfc3339(std::chrono::system_clock::time_point(
                    std::chrono::milliseconds(before->next_ms)));
                if (before->left > 0) {
        ...
        if (fired.contains("callbackReference")) {
            ...
            fired.erase(
                "callbackReference"); // "Shall be absent from Notification request messages"
...
int Worker::deliver(const StorageRef& s) {
    ...
    for (const auto& n : ctx_.store.dequeue(s, kBatch)) {
        ...
        if (ok) {
            ...
            spdlog::warn("udsf: {} notification to {} failed ({})",
                         kind,
                         req.url,
